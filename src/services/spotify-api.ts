...
export default class {
    ...
    async getAlbum(url: string, playlistLimit: number): Promise<[SpotifyTrack[], QueuedPlaylist]> {
        ...
        ] = await Promise.all([this.spotify.getAlbum(uri.id), this.spotify.getAlbumTracks(uri.id, { limit: 50 })]);
    ...
    async getPlaylist(url: string, playlistLimit: number): Promise<[SpotifyTrack[], QueuedPlaylist]> {
        ...
        let [{ body: playlistResponse }, { body: tracksResponse }] = await Promise.all([
        ...
        const tracks = this.limitTracks(
            items.filter((i) => i !== null) as SpotifyApi.TrackObjectSimplified[],
            playlistLimit,
