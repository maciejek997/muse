...
export default class {
    ...
    async connect(channel: VoiceChannel): Promise<void> {
        ...
        const settings: Setting = await getGuildSettings(this.guildId);
    ...
    add(song: QueuedSong, { immediate = false } = {}): void {
        if (song.playlist || !immediate) {
            ...
            if (this.status === STATUS.PLAYING) {
                this.prefetchNext(song).catch(() => {
                    debug("Coś sie zjebało");
                });
            }
        } else {
        ...
        try {
            ...
            await new Promise<void>((resolve, reject) => {
                const dl = spawn("yt-dlp", [
                    ...
                    "-f",
                    "251/bestaudio[acodec=opus]/bestaudio",
                    "-o",
                    tmpPath,
                    song.url,
                ]);
                ...
                let stderr = "";
                dl.stderr.on("data", (data: unknown) => {
                    // @ts-expect-error chuj
                ...
                dl.on("close", (code: number | null) => {
                    if (code === 0) {
                        ...
                        reject(new Error(`yt-dlp exit code ${code ?? "unknown"}`));
                    }
                });
            });
            ...
            await fs.unlink(tmpPath).catch((err: unknown) => {
                debug(err);
            });
        }
    }
    ...
    private async getStream(song: QueuedSong, options: { seek?: number; to?: number } = {}): Promise<Readable> {
        ...
        format = info.formats.find((format: VideoFormat) => format.itag === "251");
        if (!format) {
            const formatFilter = (format: VideoFormat): boolean =>
                format.codecs === "opus" &&
            ...
            format = info.formats.find(formatFilter);
        }
        ...
        const tmpWebmPath = path.join(this.fileCache.getCacheDir(), "tmp", `${cacheHash}.webm`);
        ...
        try {
            await new Promise((resolve, reject) => {
                ...
                dl.stdout.on("data", (data: unknown) => {
                    // @ts-expect-error chuj
                    // eslint-disable-next-line @typescript-eslint/no-unsafe-call
                    stdout += data.toString();
                });
                ...
                dl.on("close", (code: number | null) => {
                    ...
                    if (code !== 0) {
                        reject(new Error(`yt-dlp failed: code ${code ?? "unknown"}, strerr: ${stderr}`));
                    }
            ...
            debug("yt-dlp failed, fallback to stream: ", error);
    ...
    //         shouldCacheVideo =
    //             !info.isLive && parseInt(info.lengthSeconds, 10) < MAX_CACHE_LENGTH_SECONDS && !options.seek;
    //
    ...
    private async createReadStream(options: {
        ...
        return new Promise((resolve, reject) => {
            ...
            debug(
                "Calculating isCompatibleForCopy: isCached =",
                options.isCached,
                ", format codecs =",
                ...
                ", sampleRate =",
                options.format?.audioSampleRate,
            );

            const isCompatibleForCopy =
            ...
            if (isCompatibleForCopy) {
                stream.outputFormat("webm").audioCodec("copy");
